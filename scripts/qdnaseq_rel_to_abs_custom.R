...
output_dir <- "/pmglocal/jfr2137/temp/absolutecn_output_redo_24_01_02/"
bin <- 30
...
qc.data <- read.table(
  paste0(output_dir,
         "sWGS_fitting/su2c_organoids_30kb/absolute_PRE_down_sampling/su2c_organoids_fit_QC_predownsample_redo_2024-01-02.tsv"),
...
# Load combined RDS object
rds.rel <- readRDS(
  paste0(output_dir,
         "sWGS_fitting/su2c_organoids_30kb/absolute_PRE_down_sampling/su2c_organoids_30kb_relSmoothedCN.rds"))
...
if(!dir.exists(paste0(output_dir,"sWGS_fitting/",project,"_",bin,"kb/abs_cn_fullbams/plots"))){
	dir.create(paste0(output_dir,"sWGS_fitting/",project,"_",bin,"kb/abs_cn_fullbams/plots"), recursive = TRUE)
}
...
for(sample in pData(rds.rel)$name){
  ...
  if (!(is.na(ploidy[1]))) {
    ...
    # Plot abs fit
    png(paste0(output_dir,"sWGS_fitting/",project,"_",bin,"kb/abs_cn_fullbams/plots/",sample,".png"), w= 8, h = 6, unit="in", res = 250)
    par(mfrow = c(1,1))
...
# Save rds
saveRDS(abs_profiles,file=paste0(output_dir,"sWGS_fitting/",
                                 project,"_",bin,"kb/abs_cn_fullbams/",
                                 project,"_",bin,"kb_absCopyNumber_fullbams.rds"))

#write table of fits
write.table(res,paste0(output_dir,"sWGS_fitting/",
                       project,"_",bin,"kb/abs_cn_fullbams/",
                       project,"_",bin,"kb_abs_fits_fullbams.tsv"),
