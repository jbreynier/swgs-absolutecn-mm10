...
# rds.filename <- paste0(results_path, "relative_cn_rds/su2c_organoids_b0_Control_30kb_relSmoothedCN.rds")
# bin <- 30
# cores <- 2
...
res<-foreach(i=1:length(ploidies),.combine=rbind) %do%
{
        ...
        rowres<-foreach(j=1:length(purities),.combine=rbind)%do%
        {
            ...
            c(ploidy,purity,clonality,downsample_depth,downsample_depth < rds.pdata$total.reads[row.names(rds.pdata)==sample])
...
colnames(res)<-c("ploidy","purity","clonality","downsample_depth","powered")
...
# pdf(paste0(results_path, "clonality_results_custom/su2c_organoids_b0_Control_clonality.pdf"))
